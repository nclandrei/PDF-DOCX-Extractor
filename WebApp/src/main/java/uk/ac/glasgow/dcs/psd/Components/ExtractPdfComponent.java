...
/**
 * This class provides an API for extracting information from tables into CSVs
 * as well as extracting the images. The CSVs and images are then placed into
 * separate folders and a zip with both of them is returned.
 */
...
public class ExtractPdfComponent {
     ...
     * <p>
     * The process function combines the functionality of a number of helper
     * functions that make use of the Tabula jar to generate a JSON file and
     * convert that into a CSV. The Image extraction tool is then used to
     * provide all of the images from the pdf. The resulting CSVs and images are
     * then placed into a zip using the ZipMaker utility and the original files
     * are deleted.
     *
     * @param fileName the file name for the pdf to extract the tables from
     ...
     * <p>
     * generateJSON makes use of Tabula to generate a JSON file with all the
     * data from the tables in the PDF
     *
     * @param fileName the file name for the pdf to extract the tables from
     *                 (without the extension)
     */
    ...
    private static void generateJSON(final String fileName) {
        ...
        try {
            ...
            CommandLine exp =
                    parser.parse(CommandLineApp.buildOptions(), tabulaArgs);
     ...
     * <p>
     * processJSON makes use of a json file to generate a folder of CSVs where
     * each csv holds the extracted information for one table
     *
     * @param fileName the file name of the JSON file used for processing
     *                 (without the extension)
     */
